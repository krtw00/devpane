...
function prepare() {
  ...
  return {
    ...
    findExact: db.prepare(`SELECT * FROM memories WHERE category = ? AND content = ?`),
  }
}
...
export function remember(category: MemoryCategory, content: string, sourceTaskId?: string): Memory {
  const s = getStmts()
  const existing = s.findExact.get(category, content) as Memory | undefined
  if (existing) {
    const now = new Date().toISOString()
    s.update.run(existing.content, now, existing.id)
